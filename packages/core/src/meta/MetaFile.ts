import {Semaphore, useLogger} from '../utils/index.js';
import type {MetaField} from './MetaField.js';
...
export class MetaFile<T> {
  ...
  private async saveData(data: T) {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      import.meta.hot!.send('canvas-commons:meta', {
        source,
        data,
      });
    });
  ...
  static {
    if (import.meta.hot) {
      import.meta.hot.on('canvas-commons:meta-ack', ({source}) => {
