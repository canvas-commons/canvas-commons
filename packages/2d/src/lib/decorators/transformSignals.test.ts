...
describe('Curried Transform Signals', () => {
  ...
  describe('Rotation relativeTo curried API', () => {
    test('should return curried signal when called with only node', () => {
      ...
      expect(typeof curried).toBe('function');
      // Rotation doesn't have x/y components since it's scalar
      expect(curried.x).toBeUndefined();
      expect(curried.y).toBeUndefined();
    });
  });
  ...
  describe('Consistency across signal types', () => {
    test('should provide consistent API across position, scale, and rotation', () => {
      ...
      // Scalar signal should not have component access
      expect(rotationCurried.x).toBeUndefined();
      expect(rotationCurried.y).toBeUndefined();
    });
  ...
  describe('Layout Origin Signals', () => {
    ...
    test('should support relativeTo for layout origin signals', () => {
      ...
      // For layout origin signals, relativeTo works with overloaded methods, not curried
      // Test getting relative position
      const relativePos = layout.left.relativeTo(reference);
      expect(relativePos).toBeInstanceOf(Vector2);

      // Test setting relative position
      expect(() => {
        layout.left.relativeTo(reference, [50, 25]);
      }).not.toThrow();

      // Test animated relative setting
      expect(() => {
        layout.left.relativeTo(reference, [75, 50], 1);
      }).not.toThrow();
    });
    ...
    test('should work with different layout configurations', () => {
      ...
      // Origin signals should still work
      expect(() => {
      ...
      // Test with top-left origin
      const topLeftLayout = new Layout({
        size: [200, 100],
        position: [0, 0],
        origin: [-1, -1], // Top-left origin
      });
  ...
  describe('Advanced Component Access and Chaining', () => {
    test('should support complex method chaining patterns', () => {
      // Test chaining across different signal types and coordinate spaces
      const result = child.position
        .abs([100, 100])
        .scale.relativeTo(parent)([1.5, 2])
        .rotation(45)
        .position.view.x(200)
        .scale.local.y(3);
      ...
      // Verify values were set through chaining
      expect(child.position.abs().x).toBeCloseTo(200, 0.01);
      expect(child.rotation()).toBeCloseTo(45, 0.01);
    });

    test('should support nested component access patterns', () => {
      // Test accessing components through different coordinate systems
      const absX = child.position.abs.x;
      const viewY = child.position.view.y;
      const localX = child.position.local.x;

      // All should be functions
      expect(typeof absX).toBe('function');
      expect(typeof viewY).toBe('function');
      expect(typeof localX).toBe('function');

      // Should be able to get and set through these references
      const originalX = absX();
      absX(150);
      expect(absX()).not.toBe(originalX);

      viewY(200);
      expect(typeof viewY()).toBe('number');

      localX(250, 1); // With duration
      expect(typeof localX()).toBe('number');
    });

    test('should support component tweening with different timing functions', () => {
      // Test various timing scenarios
      expect(() => {
        child.position.abs.x(100, 0.5); // Short duration
      }).not.toThrow();

      expect(() => {
        child.position.view.y(200, 2.5); // Long duration
      }).not.toThrow();

      expect(() => {
        child.scale.local.x(1.5, 0); // Instant
      }).not.toThrow();

      expect(() => {
        child.position.relativeTo(parent).x(300, 1);
      }).not.toThrow();
    });
    ...
    test('should support component access with relativeTo chaining', () => {
      ...
      expect(typeof relativePosition.x()).toBe('number');
      expect(typeof relativePosition.y()).toBe('number');
      expect(typeof relativeScale.x()).toBe('number');
      expect(typeof relativeScale.y()).toBe('number');
    });
    ...
    test('should support component access return type consistency', () => {
      // Test that component methods return expected types
      const position = child.position.abs;
      const scale = child.scale.view;
      // Note: rotation signals don't have components since they're scalar

      // Getters should return numbers
      expect(typeof position.x()).toBe('number');
      expect(typeof position.y()).toBe('number');
      expect(typeof scale.x()).toBe('number');
      expect(typeof scale.y()).toBe('number');

      // Setters should return the owner
      expect(position.x(100)).toBe(child);
      expect(position.y(200)).toBe(child);
      expect(scale.x(1.5)).toBe(child);
      expect(scale.y(2)).toBe(child);

      // Tweeners should return the owner (in actual implementation these return ThreadGenerator, but we test they don't throw)
      expect(() => {
        position.x(150, 1);
        position.y(250, 1);
        scale.x(2.5, 1);
