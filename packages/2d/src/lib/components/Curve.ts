...
export abstract class Curve extends Shape {
  ...
  @initial(false)
  @signal()
  declare public readonly startArrow: SimpleSignal<boolean | ArrowDrawer, this>;
  ...
  @initial(false)
  @signal()
  declare public readonly endArrow: SimpleSignal<boolean | ArrowDrawer, this>;
  ...
  @initial(0)
  @signal()
  declare public readonly tickLength: SimpleSignal<number, this>;
  ...
  @initial(100)
  @signal()
  declare public readonly tickEvery: SimpleSignal<number, this>;
  ...
  @initial(0)
  @signal()
  declare public readonly tickOffset: SimpleSignal<number, this>;
  ...
  @initial(null)
  @signal()
  declare public readonly tickWidth: SimpleSignal<number, this>;
  ...
  @initial(null)
  @colorSignal()
  declare public readonly tickColor: ColorSignal<this>;
  ...
  @initial(0)
  @signal()
  declare public readonly tickStart: SimpleSignal<number, this>;
  ...
  @initial(1)
  @signal()
  declare public readonly tickEnd: SimpleSignal<number, this>;
  ...
  public tickPositions(): CurvePoint[] {
    return this.tickData().map(({point}) => point);
  }

  @computed()
  private tickData(): {point: CurvePoint; scale: number}[] {
    ...
    const result: {point: CurvePoint; scale: number}[] = [];
    ...
    for (let d = tickOffset; d < offsetLen; d += tickEvery) {
      ...
      result.push({point: this.getPointAtDistance(d), scale});
    }
  ...
  private drawTicks(context: CanvasRenderingContext2D) {
    ...
    const halfTick = this.tickLength() / 2;
    ...
    for (const {point, scale} of ticks) {
      const {position, normal} = point;
      const scaledHalf = halfTick * scale;

      context.beginPath();
      context.moveTo(
        position.x + normal.x * scaledHalf,
        position.y + normal.y * scaledHalf,
      );
      context.lineTo(
        position.x - normal.x * scaledHalf,
        position.y - normal.y * scaledHalf,
