...
import source from '!!raw-loader!@canvas-commons/examples/src/scenes/random';
...
unpredictability in your animation. In Canvas Commons, it's achieved using the
...
`floatArray()` and `intArray()` produce arrays of random numbers in one call,
which is handy when you want to lay out a row of nodes whose values come from a
seed:
