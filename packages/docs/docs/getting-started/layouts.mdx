...
import layoutGroupSource from '!!raw-loader!@canvas-commons/examples/src/scenes/layout-group';
import layoutSource from '!!raw-loader!@canvas-commons/examples/src/scenes/layout';
...
const rect = new Rect({
  ...
  children: [
    // ↓ layout child
    new Circle({width: 320, height: 320}),
  ],
});
```
...
class has additional `size` and `anchor` properties:
...
<ApiSnippet url={'/api/2d/components/Layout#anchor'} />
<hr />
...
export default makeScene2D(function* (view) {
  ...
  view.add([rect, yellowRect, redRect]);

  // Position the yellow rect relative to the main rect
  yellowRect.rotation(rect.rotation);
  yellowRect.right(rect.left);

  // Position the red rect relative to the main rect
  redRect.bottomLeft(rect.bottomRight);
...
const layout = new Layout({
  ...
  children: [
    new Node({
      opacity: 0.1,
      children: [
        new Rect({height: 240, fill: '#ff6470'}),
        new Rect({height: 240, fill: '#ff6470'}),
      ],
    }),
    new Rect({height: 240, fill: '#ff6470'}),
...
<Fiddle mode="preview" ratio={'2.2'} width={'1920'}>
  {layoutGroupSource}
</Fiddle>
