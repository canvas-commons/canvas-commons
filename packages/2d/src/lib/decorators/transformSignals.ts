...
class TransformConverter {
  /**
   * Convert an absolute position value to local coordinate space.
   */
  public static absoluteToLocalPosition(
    owner: Node,
    absoluteValue: SignalValue<PossibleVector2>,
  ): SignalValue<PossibleVector2> {
    if (typeof absoluteValue === 'function') {
      return () => {
        const absValue = new Vector2(absoluteValue());
        return absValue.transformAsPoint(owner.worldToParent());
      };
    }
    const absValue = new Vector2(absoluteValue);
    return absValue.transformAsPoint(owner.worldToParent());
  ...
  /**
   * Convert an absolute scale value to local coordinate space.
   */
  public static absoluteToLocalScale(
    ...
    if (typeof absoluteValue === 'function') {
      return () => {
        ...
        return absValue.div(parentAbsScale);
      };
    }
    const absValue = new Vector2(absoluteValue);
    const parentAbsScale = owner.parent()?.absoluteScale() ?? Vector2.one;
  ...
  /**
   * Convert an absolute rotation value to local coordinate space.
   */
  public static absoluteToLocalRotation(
    ...
    if (typeof absoluteValue === 'function') {
      return () => {
        const absValue = absoluteValue();
        const parentAbsRotation = owner.parent()?.absoluteRotation() ?? 0;
        return absValue - parentAbsRotation;
      };
    }
    const absValue = absoluteValue;
    const parentAbsRotation = owner.parent()?.absoluteRotation() ?? 0;
  ...
  /**
   * Convert a relative position value to absolute coordinate space.
   */
  public static relativeToAbsolutePosition(
    ...
    if (typeof relativeValue === 'function') {
      return () => {
        const relValue = new Vector2(relativeValue());
        const targetAbsPosition = targetNode.absolutePosition();
        return relValue.add(targetAbsPosition);
      };
    }
    const relValue = new Vector2(relativeValue);
    const targetAbsPosition = targetNode.absolutePosition();
  ...
  /**
   * Convert a relative scale value to absolute coordinate space.
   */
  public static relativeToAbsoluteScale(
    ...
    if (typeof relativeValue === 'function') {
      return () => {
        ...
        return relValue.mul(targetAbsScale);
      };
    }
    const relValue = new Vector2(relativeValue);
    const targetAbsScale = targetNode.absoluteScale();
  ...
  /**
   * Convert a relative rotation value to absolute coordinate space.
   */
  public static relativeToAbsoluteRotation(
    ...
    if (typeof relativeValue === 'function') {
      return () => {
        const relValue = relativeValue();
        const targetAbsRotation = targetNode.absoluteRotation();
        return relValue + targetAbsRotation;
      };
    }
    const relValue = relativeValue;
    const targetAbsRotation = targetNode.absoluteRotation();
  ...
  /**
   * Convert a view space position value to local coordinate space.
   */
  public static viewToLocalPosition(
    ...
    if (typeof viewValue === 'function') {
      return () => {
        const vValue = new Vector2(viewValue());
        // Transform view coordinates to world, then world to local
        const worldPos = vValue.transformAsPoint(owner.view().localToWorld());
        return worldPos.transformAsPoint(owner.worldToLocal());
      };
    }
    const vValue = new Vector2(viewValue);
    // Transform view coordinates to world, then world to local
  ...
  /**
   * Convert a view space scale value to local coordinate space.
   */
  public static viewToLocalScale(
    ...
    if (typeof viewValue === 'function') {
      return () => {
        ...
        const [xMagnitude, yMagnitude] =
          TransformConverter.getViewScaleMagnitudes(viewMatrix);
        return new Vector2(vValue.x / xMagnitude, vValue.y / yMagnitude);
      };
    }
    const vValue = new Vector2(viewValue);
    const viewMatrix = owner.view().localToWorld();
  ...
  /**
   * Convert a view space rotation value to local coordinate space.
   */
  public static viewToLocalRotation(
    ...
    if (typeof viewValue === 'function') {
      return () => {
        const vValue = viewValue();
        const viewRotation = TransformConverter.getViewRotation(owner);
        return vValue - viewRotation;
      };
    }
    const vValue = viewValue;
    const viewRotation = TransformConverter.getViewRotation(owner);
  ...
  /**
   * Helper function to get the view rotation from a node's transformation matrix.
   */
  ...
  /**
   * Helper function to calculate view space scale from local scale.
   */
  ...
  /**
   * Helper function to calculate scale magnitude components for view space conversion.
   */
  private static getViewScaleMagnitudes(viewMatrix: any): [number, number] {
    return [
  ...
  /**
   * Convert an absolute position value to local coordinate space for layout positions.
   * This variant uses worldToLocal() instead of worldToParent() for layout-specific transforms.
   */
  public static absoluteToLocalLayoutPosition(
    ...
    if (typeof absoluteValue === 'function') {
      return () => {
        const absValue = new Vector2(absoluteValue());
        return absValue.transformAsPoint(owner.worldToLocal());
      };
    }
    const absValue = new Vector2(absoluteValue);
    return absValue.transformAsPoint(owner.worldToLocal());
...
// Component-wise transform method interface
...
// Argument length constants for clarity
const GETTER_ARGS = 0;
const SETTER_ARGS = 1;
  ...
  public constructor(
    ...
    Object.defineProperty(this.invokable, 'abs', {
      value: createTransformSignal<PossibleVector2, Vector2, TOwner>(
        ...
        local => new Vector2(local).transformAsPoint(this.owner.localToWorld()),
        absolute =>
    ...
    Object.defineProperty(this.invokable, 'abs', {
      value: createTransformSignal<PossibleVector2, Vector2, TOwner>(
        ...
        (value, duration, timingFunction, interpolationFunction) =>
          this.invoke(value, duration, timingFunction, interpolationFunction),
        local =>
          new Vector2(local).mul(
            this.owner.parent()?.absoluteScale() ?? Vector2.one,
          ),
        absolute =>
...
> extends SignalContext<number, number, TOwner> {
  ...
  public abs(
    ...
    if (arguments.length === 0) {
      return this.owner.absoluteRotation();
    }
